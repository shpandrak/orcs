...
public class PsbAppServiceIdGeneratorTest {
    ...
    public void testItNowBye() {
        ...
        ret = PsbAppServiceIdGenerator.generatePsbAppServiceIdsByAppSvcName(
                "cool app",
                new HashSet<>(
                        Arrays.asList("svc long shitty name a", "svc long shitty name b")),
                12 + 11
        );
        ret.values().forEach(s -> Assert.assertTrue(s + " too long", s.length() <= 12 + 11));
        System.out.println(ret);
        Assert.assertEquals("cool-a-svc-l", ret.get("svc long shitty name a").substring(0, "cool-a-svc-l".length()));
        Assert.assertEquals("cool-a-svc-1", ret.get("svc long shitty name b").substring(0, "cool-a-svc-1".length()));
    }
