 ...
 * Drink responsibly
 */
public abstract class RemoteDevModeHelper {
    private static final Logger logger = LoggerFactory.getLogger(RemoteDevModeHelper.class);
    ...
    public static void registerServiceDependencies(
        ...
        for (DatasourceDescriptor currDSDescriptor : context.getDatasourceManager().getDescriptors()) {
            ...
            //todo:remote create schema on server
            RemoteH2DatasourceConfiguration remoteH2DatasourceConfiguration = new RemoteH2DatasourceConfiguration(
                    currDSDescriptor.getName(),
                    makeSchemaSafe(currDSDescriptor.getName()),
                    null,
                    null);
            DataSource dataSource = devResourceProvider.getDataSource(remoteH2DatasourceConfiguration);
            ...
            serviceRegistryAPI.registerDataSource(
                    ...
                    ResourceConfiguration.asSpecificConfiguration(
                            RemoteH2DatasourceConfiguration.class,
                            propertyValues));
        ...
        for (BlobStoreDescriptor currBlobStoreDec : context.getBlobStoreManager().getDescriptors()) {
            Map<String, String> propertyValues = overrideResourceProperties(
                    ...
                    new RemoteDevBlobStoreConfiguration(currBlobStoreDec.getName()).getPropertyValues(),
                    overriddenResourceProperties);
            ...
            serviceRegistryAPI.registerBlobStore(
                    currBlobStoreDec.getName(),
                    ResourceConfiguration.asSpecificConfiguration(
                            RemoteDevBlobStoreConfiguration.class,
                            propertyValues));
            blobstoreConfig.put(
                    currBlobStoreDec.getName(),
                    new ServiceConfig.DataSourceConfig(3, Collections.emptyMap()));
        ...
        for (Map.Entry<String, Map<String, String>> currMessagingConf : messagingConfiguration.entrySet()) {
            ...
            serviceRegistryAPI.registerQueue(
                    currMessagingConf.getKey(),
                    ResourceConfiguration.asSpecificConfiguration(RemoteDevQueueConfiguration.class, propertyValues));
        }
        ...
        serviceRegistryAPI.registerServiceConfig(context.getMicroServiceBaseURI(), ServiceConfig.generateServiceConfig(
                context.getMicroServiceBaseURI(),
                null,
                "http://localhost:" + port + "/" + context.getServiceDescriptor().getIdentifier().getRestURI(),
                "DEBUG",
    ...
    private static void addQueueProps(Map<String, Map<String, String>> messagingConfiguration, String queueURI) {
        ...
        messagingProps.putAll(new RemoteDevQueueConfiguration(QueueConfiguration.MessageDestinationType.QUEUE)
                .getPropertyValues());
    }
