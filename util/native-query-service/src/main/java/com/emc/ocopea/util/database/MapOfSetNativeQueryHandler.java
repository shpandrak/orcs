 ...
 * Time: 10:06 AM
 */
public class MapOfSetNativeQueryHandler<KeyT, ValueT> implements NativeQueryHandler<Map<KeyT, Set<ValueT>>> {
    ...
    public MapOfSetNativeQueryHandler(
    ...
    @Override
    public Map<KeyT, Set<ValueT>> handleNativeQueryResult(ResultSet rset) throws SQLException, NativeQueryException {
        Map<KeyT, Set<ValueT>> map = new HashMap<KeyT, Set<ValueT>>();
        ...
        while (rset.next()) {
            ...
            Set<ValueT> existingList = map.get(keyValue);
            if (existingList == null) {
                existingList = new HashSet<ValueT>();
                map.put(keyValue, existingList);
            }
            if (existingList.add(value)) {
